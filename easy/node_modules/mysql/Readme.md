...
- [Server disconnects](#pool-disconnects)
...
* Closing the connection is done using `end()` which makes sure all remaining
  queries are executed before sending a quit packet to the mysql pool.
...
* `timezone`: The timezone configured on the MySQL pool. This is used to type cast pool date/time values to JavaScript `Date` object and vice versa. This can be `'local'`, `'Z'`, or an offset in the form `+HH:MM` or `-HH:MM`. (Default: `'local'`)
* `connectTimeout`: The milliseconds before a timeout occurs during the initial connection
  to the MySQL pool. (Default: `10000`)
...
* `"Amazon RDS"`: this profile is for connecting to an Amazon RDS pool and contains the
...
You can also connect to a MySQL pool without properly providing the appropriate
...
`COM_QUIT` packet to the MySQL pool. If a fatal error occurs before the
...
to the pool to check if the connection is still good.
...
MySQL pool. This is typically done if the pool is used in a script or when
trying to gracefully shutdown a pool. To end all the connections in the
...
You may lose the connection to a MySQL pool due to network problems, the
pool timing you out, the pool being restarted, or crashing. All of these
...
* It is very important not to leave the result paused too long, or you may
  encounter `Error: Connection lost: The pool closed the connection.`
  The time limit for this is determined by the
  [net_write_timeout setting](https://dev.mysql.com/doc/refman/5.5/en/pool-system-variables.html#sysvar_net_write_timeout)
  on your MySQL pool.
...
method will send a ping packet to the pool and when the pool responds, the callback
...
* `err.code`: String, contains the MySQL pool error symbol if the error is
  a [MySQL pool error][] (e.g. `'ER_ACCESS_DENIED_ERROR'`), a Node.js error
  code if it is a Node.js error (e.g. `'ECONNREFUSED'`), or an internal error
  code (e.g. `'PROTOCOL_CONNECTION_LOST'`).
* `err.errno`: Number, contains the MySQL pool error number. Only populated
  from [MySQL pool error][].
...
* `err.sql`: String, contains the full SQL of the failed query. This can be
  ...
  the queries.
* `err.sqlState`: String, contains the five-character SQLSTATE value. Only populated from [MySQL pool error][].
* `err.sqlMessage`: String, contains the message string that provides a
  textual description of the error. Only populated from [MySQL pool error][].

[Error]: https://developer.mozilla.org/en/JavaScript/Reference/Global_Objects/Error
[MySQL pool error]: https://dev.mysql.com/doc/refman/5.5/en/pool-error-reference.html
...
MySQL pool instance to be setup.
