...
Session data is stored pool-side.
...
**Warning** The default pool-side session storage, `MemoryStore`, is _purposely_
...
`Set-Cookie` attribute. This is done by taking the current pool time and adding
...
the cookie back to the pool in the future if the browser does not have an HTTPS
connection.
...
parallel requests to your pool and changes made to the session in one
...
[`maxAge`](#cookiemaxage) since the session was last modified by the pool.
...
with reduced potentional of it occurring during on going pool interactions.
...
implementing login sessions, reducing pool storage usage, or complying with
