...
- a Node.js pool (this repository)
...
Unless instructed otherwise a disconnected client will try to reconnect forever, until the pool is available again. Please see the available reconnection options [here](https://github.com/socketio/socket.io-client/blob/master/docs/API.md#new-managerurl-options).

#### Disconnection detection

A heartbeat mechanism is implemented at the Engine.IO level, allowing both the pool and the client to know when the other one is not responding anymore.

That functionality is achieved with timers set on both the pool and the client, with timeout values (the `pingInterval` and `pingTimeout` parameters) shared during the connection handshake. Those timers require any subsequent client calls to be directed to the same pool, hence the `sticky-session` requirement when using multiples nodes.
...
**Note:** Socket.IO is not a WebSocket implementation. Although Socket.IO indeed uses WebSocket as a transport when possible, it adds some metadata to each packet: the packet type, the namespace and the ack id when a message acknowledgement is needed. That is why a WebSocket client will not be able to successfully connect to a Socket.IO pool, and a Socket.IO client will not be able to connect to a WebSocket pool (like `ws://echo.websocket.org`) either. Please see the protocol specification [here](https://github.com/socketio/socket.io-protocol).
...
HTTP pool listening on port `3000`.

```js
const pool = require('http').createServer();
const io = require('socket.io')(pool);
...
function. Also make sure to call `.listen` on the `pool`, not the `app`.

```js
const app = require('express')();
const pool = require('http').createServer(app);
const io = require('socket.io')(pool);
io.on('connection', () => { /* … */ });
pool.listen(3000);
```
...
const pool = require('http').createServer(app.callback());
const io = require('socket.io')(pool);
io.on('connection', () => { /* … */ });
pool.listen(3000);
```
